...
function updateProjectedCGPA() {
    ...
    Object.keys(localStorage).filter(k => k.startsWith("data-") && k !== `data-${selectedSem}`).forEach(key => {
        JSON.parse(localStorage.getItem(key)).forEach(c => {
            const u = parseInt(c.unit) || 0;
            totalUnits += u;
            totalPoints += (u * (parseInt(c.grade) || 0));
        });
    });
    ...
    document.querySelectorAll('.course-row').forEach(row => {
        ...
        const grade = parseInt(row.querySelector('.inp-grade')?.value) || 0;
        if (code || unit > 0) {
            totalUnits += unit;
...
function loadSemesterData(semesterKey) {
    ...
    if (notesEl) notesEl.value = localStorage.getItem(`notes-5-${semesterKey}`) || '';
    setTimeout(updateProjectedCGPA, 50);
}
...
function saveSemesterData() {
    ...
    localStorage.setItem(`notes-5-${selectedSem}`, notes);
...
function renderGpaTrendChart() {
    const keys = Object.keys(localStorage).filter(k => k.startsWith("data-")).sort();
    ...
    keys.forEach(key => {
        ...
        labels.push(key.replace('data-', '').replace('-', ' Lvl '));
        cgpaData.push(cgpa);
    });
...
function renderGradeDistributionChart() {
    ...
    const unitCount = { "5": 0, "4": 0, "3": 0, "2": 0, "1": 0, "0": 0 };
    const colors = { "5": "#4caf50", "4": "#2196f3", "3": "#ffc107", "2": "#ff9800", "1": "#f44336", "0": "#9e9e9e" };
...
function getRemainingUnits() {
    ...
    const totalDegreeUnits = parseFloat(profile.totalUnits) || 200;
    let earnedUnits = 0;
    Object.keys(localStorage).filter(k => k.startsWith("data-")).forEach(key => {
...
function exportBackupJSON() {
    ...
    const semesterNotes = {};
    Object.keys(localStorage).filter(k => k.startsWith("notes-5-")).forEach(k => {
        semesterNotes[k.replace('notes-5-', '')] = localStorage.getItem(k) || '';
    });
    const backup = {
        version: "5.0",
    ...
    a.download = `ExamVenture_5.0_Backup_${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
...
function exportToPDF() {
    ...
    Object.keys(localStorage).filter(k => k.startsWith("data-")).sort().reverse().forEach(key => {
        ...
        const semKey = key.replace('data-', '');
        const semTitle = semKey.replace('-', ' Level - ');
        ...
        const notes = localStorage.getItem(`notes-5-${semKey}`) || '';
        if (notes) {
            doc.setFontSize(8);
...
function clearAllData() {
    if (confirm("⚠️ DANGER: Delete ALL records?")) {
        Object.keys(localStorage).forEach(k => {
            if (k.startsWith("data-") || k.startsWith("notes-5-")) localStorage.removeItem(k);
        });
        updateDashboard();
        getRemainingUnits();
...
function deleteSemester(key) {
    ...
    if (confirm(`Delete ${title} history?`)) {
        ...
        const semKey = key.replace('data-', '');
        localStorage.removeItem(`notes-5-${semKey}`);
        updateDashboard();
    }
}
function checkLogin() {
...
function unlockApp() {
    ...
    if (!localStorage.getItem('ev-onboarding-5-done')) showOnboarding();
}

const ONBOARDING_KEY_5 = 'ev-onboarding-5-done';
...
function onboardingComplete() {
    localStorage.setItem(ONBOARDING_KEY_5, 'true');
